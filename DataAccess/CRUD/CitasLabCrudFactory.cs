...
{
    public class CitasLabCrudFactory : CrudFactory
    {
        private CitasLaboratorioMapper mapper;

        public CitasLabCrudFactory() : base()
        {
            mapper = new CitasLaboratorioMapper();
            dao = SqlDao.GetInstance();
        }
        public override string CreateRespuesta(EntidadBase entityDTO)
        {
            var pedido = (CitaLaboratorio)entityDTO;
        ...
        public  void DeleteId(int pId)
        {
            ...
            var sqlOperation = mapper.DeleteStatement(pId);
        ...
        public  List<T> RetrieveAllById<T>(int id)
        {
            var list = new List<T>();
