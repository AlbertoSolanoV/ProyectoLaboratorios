...
{
    internal class UsuarioActivoMapper : ICrudStatementsReinicioContraseña, IObjectMapper
    {
        ...
        public SqlOperation GetRetrieveByStringStatement(string correo)
        {
            var operation = new SqlOperation
            {
                ProcedureName = "sp_devolver_usuario_correo"
            };
            operation.AddVarcharParam("emailUsuario", correo);
        ...
        public SqlOperation GetUpdateStatement(EntidadBase entidadDTO)
        {
            var operation = new SqlOperation()
            {
                ProcedureName = "sp_modificar_usuario_estado_activo"

            };

            var activarUsuario = (Usuario)entidadDTO;
            
            operation.AddIntegerParam("idUsuario", activarUsuario.id);
            operation.AddVarcharParam("usuarioActivo", activarUsuario.usuarioActivo);
